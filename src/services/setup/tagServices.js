...
export const TagServices = {
	fetchTags: async (page = "all", keyword = null) => {
		try {
			let queryString = "";
			if (!page) {
				page = 1;
			...
			const result = await axios.get(config.tagUrl + "/list" + queryString, {
				headers: headers,
			});
			return result.data;
		} catch (e) {
	...
	saveTag: async (payload) => {
		try {
			...
			console.log("save tag", result.data);
			return result.data;
		} catch (e) {
			if (e.response && e.response.data && e.response.data.message) {
				console.log(e.response.data.message);
	...
	deleteTag: async (id) => {
		try {
			...
			if (result.status === 200) {
				console.log("delete tag", result.data);
				return result.data;
			} else {
				throw new Error(
