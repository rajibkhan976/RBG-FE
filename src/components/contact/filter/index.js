...
function ImportFilter(props) {
	...
	useEffect(() => {
		if (tagListData && tagListData.tags) {
			setTagsData(tagListData.tags);
			setSearchResult(tagListData.tags);
			setIsLoader(false);
		}
	}, [tagListData]);

	// let filterData = useSelector((status) => status.filter);
	// console.log("fetch phase api is calling ==============================", filterData);
	// Tags open
	const openTagHandel = () => {
		setTagIsOpen(true);
	...
	const selectedTagHandeler = (event, item, index) => {
		...
		if (event.target.checked) {
			...
			selectedArray = tagsData.map((item) =>
				item._id === event.target.value ? { ...item, checked: true } : item
			);
			...
			selectedArray = tagsData.map((item) =>
				item._id === event.target.value ? { ...item, checked: false } : item
			);
			setSearchTags("");
	...
	const tagSearchHandeler = (event) => {
		...
		let filterData = tagsData.filter((item) =>
			item.name.toLowerCase().includes(event.trim())
		);
	...
	useEffect(() => {
		// console.log("Contact filter data", contactFilterData);
		const { createdBy, phase, source } = contactFilterData;
		setPhases(phase);
		setSource(source);
	...
	useEffect(() => {
		...
		if (fromDate) {
			let formateFormDate = decodeURIComponent(
				fromDate.replaceAll("+", " ")
			).trim();
			console.log(
				"From Date",
				moment(formateFormDate).format("YYYY-MM-DD HH:MM:ss")
			);
			let timeZoneConversation = utils.convertUTCToTimezone(
				moment(String(formateFormDate).trim()).format("YYYY-MM-DD HH:mm:ss"),
				timezoneOffset
			);
			console.log(
				"From date UTC to local timezone convert----------------",
				timeZoneConversation
			);
			setSelectedFrom(
				fromDate
					? moment(new Date(timeZoneConversation)).format("YYYY-MM-DD")
					: ""
			);
		...
		if (toDate) {
			let formateToDate = decodeURIComponent(
				toDate.replaceAll("+", " ")
			).trim();
			// console.log("To Date", formateToDate);
			let timeZoneConversation = utils.convertUTCToTimezone(
				moment(formateToDate).format("YYYY-MM-DD HH:MM:ss"),
				timezoneOffset
			);
			setSelectedTo(
				toDate
		...
		if (source) {
			setSelectedSource(
				source ? decodeURIComponent(source.replaceAll("+", " ")) : ""
			);
		}
		...
		if (tags && tagsData) {
			let selectTagData = tags
				? decodeURIComponent(tags.replaceAll("+", " "))
				: "";
							...
							<div className='infoInputs appModal'>
								<ul>
										...
										<div className='formField w-100 appModals formControl creationSelection tagSelection'>
															...
															.map((item, index) => {
																return (
																	...
																	>
																		...
																		<button
																			type='button'
																			onClick={() => deselectTag(item, index)}
																		>
																			<img
																				src={cressIcon}
																				alt=''
											...
											<div ref={ref}>
												<div
													className={
														tagIsOpen
															? "cz_selectCategory cmnFieldStyle focus"
															: "cz_selectCategory cmnFieldStyle"
													}
												>
									...
									<li className='dateRangeHeading'>
										<p className='dateRange pTags'>Created on</p>
									</li>
...
export default memo(ImportFilter);
