            ...
            <div className="modalBackdrop modalAddholiday">
                <div className="slickModalBody">
                    <div className="slickModalHeader">
                        <button className="topCross" onClick={props.closeModal}><img src={crossTop} alt="" /></button>
                        <div className="circleForIcon"><img src={tagIcon} alt="" /></div>
                        <h3>Tags</h3>
                        <p>Please hover on a tag to remove.</p>
                    </div>
                    <div className="modalForm auto">
                        <div className="tagContainer">
                            {tagList.length > 0 && tagList.map((tag, key) => (
                                <span key={key} class="indTags">
                                  <span class="labelSelected">{tag.name}</span>
                                  <span class="closeTag" onClick={() => deleteTag(tag._id)}><img src={cross_w} alt=""/></span>
                                </span>
                            ))}
                        </div>
                        <div className="modalbtnHolder">
                            <button type="button"
                                    ...
                                    className="saveNnewBtn"><span>Close</span><img src={arrowRightWhite}/></button>
                        </div>
