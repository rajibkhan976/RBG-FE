...
const Transaction = (props) => {
    ...
    const dateCalculationFunction = (transdate)=>{
        ...
        //modVal = modVal%(1000)
       // var mili = modVal
        //console.log( Math.floor(daydiff) + " , " + Math.floor(hours)  + " , " + Math.floor(min)  + " , " + Math.floor(sec)); //+ " , " + Math.floor(mili)); 
        var showTime ;
        if(daydiff >= 1) {
                        ...
                        transactionList.length !== 0 ? (
                           transactionList.transactions.map((transList, i) => (
                               <>
                                    ...
                                    <div key={i} className={transList.outcome.result === "success" ? "row success" : transList.outcome.result === "failed" ? "row fail" : "row"}>
                                        <div className="cell">
                                        ...
                                        <span className={transList.is_pos === true ? "amount pos" : "amount"}> 
                                             {transList.approved_amount ? "$" + transList.approved_amount : "NA"} 
                                             {transList.is_pos && <span>POS</span>}
                                        </span>
                                        </div>
                                        <div className="cell">
                                            <span className="transID">{transList.transaction_id ? transList.transaction_id : "NA"}</span>
                                        </div>
                                        <div className="cell">
                                            <span className="time">{dateCalculationFunction(transList.transaction_date)}
                                            </span>
                                        </div>
                                    </div>
