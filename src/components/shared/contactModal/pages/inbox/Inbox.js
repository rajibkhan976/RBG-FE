...
const Inbox = (props) => {
	...
	const fetchInboxLogList = async (pageId) => {
		...
		try {
			setIsLoader(true);
			setIsScroll(true);
			const result = await communicationLogServices.fetchInboxLog(
				pageId,
				contact_id
			);
			...
			if (result.pagination.page === "1") {
				comLogRef.current.scrollTop =
					comLogRef.current.scrollHeight - comLogRef.current.clientHeight;
			} else {
				comLogRef.current.scrollTop =
					comLogRef.current.scrollHeight - scrolledPosition + 260;
			}
		} catch (e) {
		} finally {
			setIsScroll(false);
	...
	const emailplaceholdingData = (data) => {
		...
		keys.map((el) => {
			subject = subject.replaceAll(
				"[" + el + "]",
				contactGenData && contactGenData[el] ? contactGenData[el] : ""
			);
	...
	const areaOfText = useRef(
		contactLogData.slice(0).reverse().map(React.createRef)
	);
			...
			<div className='contactTabsInner'>
				...
				>
					...
					scrolledTop <= scrolledHeight - clientHeight - 800 ? (
						<button
							className='goData'
							onClick={goToListBottom}
						>
							Jump to latest Messages <img src={arrowDown} />
						</button>
					) : (
						""
					)}
					{contactLogData && contactLogData.length > 0 && (
						<div className='noMoredata'>
							This contact has no more communication{" "}
						</div>
					)}
					{contactLogData &&
						contactLogData.length > 0 &&
						contactLogData
							.slice(0)
							.reverse()
							.map((elem, key) => {
									...
									<div
										className={
											elem.direction === "inbound"
												? "inboxChat incomingChat"
												: "inboxChat outgoingChat"
										}
										key={key}
									>
												...
												src={
													elem?.log_type === "SMS"
														...
														? elem?.direction === "outbound"
															? iconCallOut
															: elem?.direction === "inbound"
															? elem?.data?.status === "forwarded"
																? iconCallforword
																: iconCallIn
																? elem?.data?.callStatus === "no-answer"
																	? iconMissedCall
																	: iconCallIn
																	? elem?.data?.callStatus === "ringing"
																		? iconCallOut
																		: ""
																	: ""
																: ""
										...
										<div className='txtArea'>
											<div className='areaOfText'>
												{elem.log_type === "SMS" ? (
													...
													<div className='audioBox communicationAudioPlayer inboxAudioPlayer'>
														<Player
															audioElement={new Audio(elem.data.recordingUrl)}
															trackName={
																elem.direction +
																" " +
																elem.log_type.toLowerCase() +
																(elem.direction === "outbound"
																	? " to " + elem.to
																	: " from " + elem.from)
															}
													...
													<>
														...
														<div
															...
															dangerouslySetInnerHTML={{
																__html:
																	elem.data && elem.data.template
																		? elem.data.template.slice(
																				0,
																				280 - elem.data?.subject.length
																		  )
																		: "",
															}}
														></div>
											...
											{(elem.data?.template ? elem.data?.template.length : "") +
												(elem.data?.subject ? elem.data?.subject.length : "") >
											260 ? (
												<button
													onClick={() =>
														showBigMail(
					...
					{newEmailData.length > 0 &&
						newEmailData
							...
							.map((elem, key) => {
								return (
									<div
										className='inboxChat outgoingChat'
										key={key}
									>
										<div className='msgTypeIcon'>
											<img
												src={
													elem.log_type === "EMAIL" ? iconEmailOut : iconSmsOut
												}
												alt=''
											/>
										</div>
										<div className='txtArea'>
											...
											{(elem.template
												? utils.decodeHTML(elem.template).length
												: "") +
												(elem.subject ? elem.subject.trim().length : "") >
											267 ? (
												<button
													onClick={() => showBigMailStatic(elem, elem.log_type)}
													className='noBg'
												>
													read more
												</button>
											) : elem?.message && elem.message.trim().length > 280 ? (
												<button
													onClick={() =>
														showBigMailStatic(elem.message, elem.log_type)
													}
													className='noBg'
												>
													read more
						...
						newEmailData.length === 0 && (
							<div className='appListsWrap'>
								<div className='noDataFound'>
									<span>
										<span>This contact doesn’t have any communication yet</span>
									</span>
								</div>
							</div>
						)}
