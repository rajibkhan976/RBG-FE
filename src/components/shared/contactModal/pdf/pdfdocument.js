...
export default function PDFDocument(props) {
	const transactionData = {
		...
		orgCode: props.org.organizationCode,
	};
					...
					<div style={styles.container}>
								...
								<tr>
									...
									<td style={styles.right}>
										<div style={styles.text}>
											Invoice Number :{" "}
											<span style={styles.span1}> {txn[0].transactionId}</span>
										</div>
										<div style={styles.text}>
											Transaction Date :{" "}
						...
						<div style={styles.main}>
							...
							<div style={styles.text4}>
								...
								{contact.phone?.number
									? contact.phone.dailCode + "-" + contact.phone?.number
									: ""}
							</div>
								...
								<div style={styles.tableWrapper}>
									{txn[0].transaction_for === "product" ? (
										...
										>
											...
											<tbody>
												{Array.isArray(txn[0].transaction_data) &&
													txn[0].transaction_data.map((el, index) => {
														const price = !isRefund
															? Number(el?.price * el?.qnty)
															: txn[0].amount;
														subTotal = !isRefund
															? subTotal + price
															: txn[0].amount;
														return (
															<tr>
																...
																<td style={styles.tableQTD}>
																	{!isRefund
																		? el.product
																		: "Refund for " + el.product}
																</td>
																<td style={styles.tableQTD}>
																	{el?.color || "-"}
																</td>
										...
										>
											...
											<tbody>
												<tr>
													...
													<td style={styles.tableQTD}>
														{!isRefund
															? txn[0].transaction_data.course
															: "Refund for " + txn[0].transaction_data.course}
													</td>
													<td style={styles.tableQTD}>
														{moment(
															txn[0].transaction_data.course_start
														).format("LL")}
													</td>
													<td style={styles.tableQTD}>
														${txn[0].amount.toFixed(2)}
									...
									<tbody>
										{txn.map((el) => {
											...
											if (isRefund) {
												...
												desc =
													"Program - " +
													el?.payment_for.charAt(0).toUpperCase() +
													el?.payment_for.slice(1);
											}
											...
											return (
												<tr>
													...
													<td style={styles.tableQTD}>
														{el.payment_via.charAt(0).toUpperCase() +
															el.payment_via.slice(1)}
													</td>
													<td style={styles.tableQTD}> {el._id}</td>
													<td style={styles.tableQTD}>
														${Math.abs(el.amount).toFixed(2)}
													</td>
												</tr>
											);
										})}
											...
											{txn[0].payment_via !== "cash" && !isRefund ? (
												<td style={styles.bigfooterTD}>
													<div style={styles.bigfooterP}>
														<span style={styles.bigfooterSpan}>
															{txn[0].payment_resp.hasOwnProperty(
																"bank_account"
															)
																? "Bank Details:"
																: "Credit Card Details:"}
														...
														XXXX
														{txn[0].payment_resp.hasOwnProperty("bank_account")
															? txn[0].payment_resp.bank_account?.last4
															: txn[0].payment_resp.card?.last4}
													</div>
													<div style={styles.bigfooterP}>
														<span style={styles.bigfooterSpan}>
															{txn[0].payment_resp.hasOwnProperty(
														...
														{txn[0].payment_resp.hasOwnProperty("bank_account")
															? txn[0].payment_resp.bank_account.routing_number
															: txn[0].payment_resp.card.expiration_month +
															  "/" +
															  txn[0].payment_resp.card.expiration_year}
													</div>
												</td>
											) : (
												<td style={styles.bigfooterP}>
													{isRefund ? "Refunded via original method" : ""}
												</td>
											)}
											<td>&nbsp;</td>
											<td style={styles.text6}>
												{isRefund ? "Total Refunded" : "Total Paid"} :
											</td>
											<td style={styles.text6}>
												${Math.abs(txn[0].amount).toFixed(2)}
											</td>
										</tr>
									</tbody>
								</table>
						...
						<div style={styles.footer}>
							<div style={styles.text7}>
								Note : This is a digitally generated document and does not
								require any signature.
							</div>
							<div style={styles.text8}>
								© redbeltgym.com {date.getFullYear()}
							</div>
						</div>
					</div>
				</body>
