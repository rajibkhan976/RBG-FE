...
import { getTagListData, saveTag } from "../../actions/TagActions";
...
const TagList = (props) => {
	...
	const fetchTags = (pageNumber, keyword) => {
		try {
			setIsScroll(true);
			setTagLoader(true);
			dispatch(getTagListData(pageNumber, keyword ? keyword : searchedTag));
		} catch (e) {
			dispatch({
	...
	useEffect(() => {
		if (tagListData) {
			...
			setUpcomingPagination(tagListData.pagination);
		}
	}, [tagListData]);
	...
	const createNewTag = (e) => {
		...
		if (searchedTag.trim() !== "") {
			try {
				setIsScroll(true);
				dispatch(saveTag({ name: searchedTag }));
				setIsScroll(false);
	...
	console.log("search tag data", searchedTag);
	useEffect(() => {
		// console.log("tag data", savedTag);
		if (savedTag) {
			checkThisTag({ ...savedTag }, true);
			// props.setInitialTagsData([...props.initialTagsData, savedTag]);
		}		
	}, [savedTag]);
		...
		if (filtered.length > 0) {
			...
			return (
				<div className='noTagsFound'>
					...
					<h4>{searchedTag}</h4>
					<footer
						className='w-100 d-flex f-justify-center'
	...
	const upcomingTagPageNo = (element) => {
		if (
			!isScroll &&
			upcomingPagination.totalPages >= upcomingPagination.currentPage
		) {
			if (
				...
				if (upcomingPagination.currentPage < upcomingPagination.totalPages) {
					setTagsPage(upcomingPagination.currentPage + 1);
	...
	useEffect(()=>{
		if(props.tagListToggles){
			// console.log("open open open open", props.tagListToggles);
			dispatch(getTagListData(1, ""));
		}
	},[props.tagListToggles])
			...
			{tagListToggle ? (
				...
				>
					...
					<div
						className={
							tagNameError
								? "searchTag cmnFormField errorField"
								: "searchTag cmnFormField"
						}
					>
						<input
							type='search'
					...
					<div className='genScrollBody f-1'>
						<Scrollbars
							...
							ref={scrollBodyTags}
							onScroll={upcomingTagPageNo}
						>
							{" "}
							{/**onScroll={appoPageList} */}
