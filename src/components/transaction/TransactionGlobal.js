...
const TransactionGlobal = (props) => {
	...
	const fetchTransHistoryList = async (pageNo) => {
		try {
			...
			const response = await TransactionHistoryServices.fetchTransHistoryList(
				queryParams,
				page
			);
				...
				<div className='userListBody'>
					{oldTransactionList?.length ? (
						<div className='overviewTable tarnsGlobal'>
							...
							{oldTransactionList?.length
								? oldTransactionList.map((tHistory, key) => {
										return (
											<>
												<div
													className='listRow transGlobal'
													key={"trx_" + key}
												>
													<div
														className='listCell cellWidth_15'
														onClick={() => openContactModal(tHistory)}
													>
														...
														{tHistory.contact && tHistory.contact._id ? (
															<button className='noBg commonColor'>
																{" "}
																{tHistory?.contact?.firstName}{" "}
																{tHistory?.contact?.lastName}
															</button>
														) : (
															<button
																className='noBg commonColor'
																style={{ color: "#9BAEBC" }}
																title='Contact Deleted'
															>
																{" "}
																{tHistory?.contactDeleted?.firstName}{" "}
																{tHistory?.contactDeleted?.lastName}
															</button>
														)}
													</div>
													<div className='listCell cellWidth_15'>
														<button className='noBg'>
															<span className='blueTxt'>
																{tHistory.contact && tHistory.contact._id
																	? tHistory?.contact?.email
																	: tHistory?.contactDeleted?.email}
															</span>
														</button>
													</div>
													<div className='listCell cellWidth_15'>
													...
													<div className='listCell cellWidth_15'>
														<span className='spanned'>
															{!tHistory?.last_transaction_date
																? "N/A"
																: tHistory?.history[0]?.transactionId ||
																  tHistory?.history[0]?.transaction_id}
														</span>
													</div>
													<div className='listCell cellWidth_10'>
														<span className='boldTxt'>
															${Math.abs(tHistory?.amount).toFixed(2)}
														</span>
														{tHistory?.history[tHistory?.history?.length - 1]
															?.refunded_amount ? (
															<span className='refundAmmount'>
																-$
																{Math.abs(
																	tHistory?.history[
																		tHistory?.history?.length - 1
																	]?.refunded_amount
																).toFixed(2)}
															</span>
														) : (
													...
													<div className='listCell cellWidth_10'>
														{tHistory?.transaction_for === "course"
															? "Program"
															: "Product"}
													</div>
													<div className='listCell cellWidth_7 center'>
														{tHistory.transaction_for === "product" ? (
															<div className='proListed'>
																<span className='numberTag'>
																	{tHistory?.items?.length}
																</span>

																<div className='productDetails'>
																	{tHistory?.items &&
																		tHistory?.items.map((prolist, key2) => {
																			return (
																				<div
																					className='indiviPro'
																					key={key2}
																				>
																					<h3>
																						{prolist.product[0].toUpperCase() +
																							prolist.product.slice(1)}
																					</h3>
																					<p>
																						<span>Color : {prolist.color}</span>
																						<span>Size : {prolist.size}</span>
																						<span>Qty : {prolist.qnty}</span>
																					</p>
																				</div>
																			);
																		})}
														...
														<button
															className={
																tHistory.status === "completed"
																	? "statusBtn succes"
																	: tHistory.status === "failed" ||
																	  tHistory.status == "active"
																	? "statusBtn fail"
																	: tHistory.status === "refunded" ||
																	  tHistory.status === "overdue"
																	? "statusBtn refund"
																	: "statusBtn"
															}
														>
													...
													<div className='listCell cellWidth_5 center'>
														{tHistory?.history.length > 0 && (
															...
															>
																<img
																	src={arrowDown}
																	className={option === key && "rotateround"}
																/>
															</button>
														)}
													</div>
												...
												>
													{tHistory?.history
														? tHistory.history.map((historylist, key1) => {
																return (
																	<>
																		<div
																		...
																		>
																			...
																			<div className='listCell time'>
																				{utils.convertUTCToTimezone(
																					historylist?.transaction_date?.trim(),
																					timezoneOffset
																				)}
                                                                        ...
                                                                        <span className='doomed'>{moment(utils.convertUTCToTimezone(historylist?.transaction_date, timezone, 'YYYY-MM-DD HH:mm:ss')).format('hh:mm A')}</span> */}
																			...
																			<div className='listCell id'>
																				...
																				{historylist.amount < 0 ? (
																				  ...
																				  historylist?.payment_resp?.outcome
																						?.description ? (
																					<div className='notePop'>
																						<div className='notePopIcon'></div>
																						<div className='notePopContent'>
																			...
																			<div className='listCell value'>
																				<span className='boldTxt'>
																					$
																					{Math.abs(historylist.amount).toFixed(
																						2
																					)}
																				</span>
																			</div>
                                                            ...
                                                            {historylist.status}
																			...
																			{historylist.status === "success" ? (
																				<div className='listCell download'>
																					{tHistory?.contact._id !==
																						undefined ||
																					tHistory?.contactDeleted !==
																						undefined ? (
																						<PDFDownloadLink
																							document={
																								<PDFDocument
																									key={key1}
																									transactionData={historylist}
																									contact={
																										tHistory?.contact._id !==
																										undefined
																											? tHistory?.contact
																									...
																									org={org}
																									transactionDate={utils.convertUTCToTimezone(
																										historylist.transaction_date,
																										timezoneOffset
																									)}
																								/>
																							}
