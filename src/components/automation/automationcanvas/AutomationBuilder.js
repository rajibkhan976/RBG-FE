...
const AutomationBuilder = (props) => {
	...
	const onNodeMouseEnter = (event, node) => {
		...
		deleteButton.addEventListener("click", (event) => deleteNode(event, node));
		switch (thisNodeType) {
			case "Trigger":
			...
			case "ActionEmail":
				...
				thisNodeTarget.closest(".react-flow__node").appendChild(editButton);
				thisNodeTarget
					.closest(".react-flow__node-actionEmail")
			...
			case "ActionMessage":
				...
				thisNodeTarget.closest(".react-flow__node").appendChild(editButton);
				thisNodeTarget
					.closest(".react-flow__node-actionMessage")
					.appendChild(deleteButton);
				break;
	...
	const refreshWebhook = async (id, nodeI) => {
		...
		if (id) {
			...
			setIsLoader(true);
			let fetchFields = await AutomationServices.fetchFields(
		...
		if (elementsToRemove.length) {
			...
			updatedElem.map((elem) => {
				if (
					...
					elem.data.nodes.previous.includes(elementId)
				) {
					elem.data.nodes.previous = "";
				}
				if (
					...
					elem.data.nodes &&
					elem.data.nodes &&
					elem.data.nodes.next &&
					elem.data.nodes.next.includes(elementId)
				) {
		...
		try {
			if (!automationName.trim()) {
				...
				const brokenElem = bluePrintElem.find((el) => {
					if (
						(el.type !== "fieldActionTrigger" &&
							el.data.nodes.previous === "" &&
							el.type !== "trigger" &&
							el.data.nodes.previous === "" &&
							el.type !== "attendanceTrigger" &&
							el.data.nodes.previous === "" &&
							el.type !== "appointmentTrigger" &&
							el.data.nodes.previous === "" &&
							el.type !== "transactionTrigger" &&
							el.data.nodes.previous === "" &&
							el.type !== "tagTrigger" &&
							el.data.nodes.previous === "") ||
						(el.type === "fieldActionTrigger" &&
							el.data.nodes.next.length === 0 &&
							el.type === "trigger" &&
							el.data.nodes.next.length === 0 &&
							el.type === "attendanceTrigger" &&
							el.data.nodes.next.length === 0 &&
				...
				if (typeof brokenElem == "undefined") {
					...
					try {
						if (automationId) {
							...
							await AutomationServices.deleteArn(JSON.stringify(payloadArn));
						}
					} catch (e) {
						console.log(e);
					}
					...
					dispatch({
						type: actionTypes.SHOW_MESSAGE,
						message:
							"You have broken element. Make sure to connect all the nodes",
						typeMessage: "error",
					});
				}
			}
	...
	const onDrop = async (event) => {
		...
		if (filteredElements.length) {
			if (
				type === "trigger" ||
				type === "fieldActionTrigger" ||
				type === "appointmentTrigger" ||
				type === "attendanceTrigger" ||
			...
			if (
				type === "actionDelay" ||
				type === "actionStatusPhaseUpdate" ||
				type === "filter" ||
				type === "actionApplyTag" ||
				type === "actionRemoveTag" ||
				type === "actionMessage" ||
				type === "actionEmail" ||
	...
	const getConnectionDetails = (source, target) => {
		...
		if (
			sourceType === "actionMessage" ||
			sourceType === "actionEmail" ||
			sourceType === "actionDelay" ||
			sourceType === "actionStatusPhaseUpdate" ||
			sourceType === "actionApplyTag" ||
			sourceType === "actionRemoveTag" ||
			sourceType === "notificationTag" ||
			sourceType === "automationTag"
		) {
			sourceType = "action";
		}
		if (
			targetType === "actionMessage" ||
			targetType === "actionEmail" ||
			targetType === "actionDelay" ||
			targetType === "actionStatusPhaseUpdate" ||
			targetType === "actionApplyTag" ||
			targetType === "actionRemoveTag" ||
			targetType === "notificationTag" ||
			targetType === "automationTag"
		) {
			targetType = "action";
		}
	...
	const copyTag = (text, field) => {
		...
		if (e.target && e.target.name === field) {
			...
			let textBeforeCursorPosition = e.target.value.substring(
				0,
				cursorPosition
			);
		...
		if (id && data.source && data.target) {
			...
			updatedElem.map((elem) => {
				if (
					...
					elem.data.nodes &&
					elem.data.nodes.next &&
					elem.data.nodes.next.includes(data.source)
				) {
					let value2 = elem.data.nodes.next;
				...
				if (
					...
					elem.data.nodes &&
					elem.data.nodes.previous &&
					elem.data.nodes.previous === data.target
				) {
					elem.data.nodes.previous = "";
				}
			});
					...
					>
						<div className='automationRow'>
							<div className='automationTopLeft'>
								<input
									type='text'
									className={`automationNameArea ${automationNameError}`}
								...
								<div className='automationPublish'>
									<button
										className='automationPublishBtn'
							...
							case "webhookTrigger":
								return (
									<WebhookTrigger
										...
										automationUrlId={automationUrlId}
							...
							case "actionDelay":
								return (
									<Delay
										...
										triggerNodeId={triggerNodeId}
										saveDelay={saveDelay}
										handleDelayTypeChange={handleDelayTypeChange}
										delayType={delayType}
							...
							case "filter":
								return (
									<Filter
										...
										phaseList={phaseList}
										deleteNodeFiler={deleteNodeFiler}
										addAnd={addAnd}
										addOr={addOr}
							...
							case "automationTag":
								return (
									<AutomationModal
										...
										elem={tagNodeId}
										automationId={automationActionId}
