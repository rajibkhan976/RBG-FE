...
import GiftPackageDrawer from "./GiftPackageDrawer";
...
const PackageSetup = () => {
	...
	const [toggleGiftPkgDrawer, setToggleGiftPkgDrawer] = useState(false);
		...
		try {
			...
			dispatch({
				...
				message: `Package status ${
					creditPackages[key].status ? "activated" : "deactivated"
				} successfully.`,
				typeMessage: "success",
			});
		} catch (e) {
	...
	const toggleFavouritePackage = async (id, key) => {
		console.log(
			"Toggle favourite package",
			id,
			creditPackages[key].isFavourite
		);
		try {
			...
			dispatch({
				type: actionTypes.SHOW_MESSAGE,
				message: `Package ${
					creditPackages[key].isFavourite ? "added as" : "removed from"
					...
					<div className='userListHead'>
						<div className='listInfo'>
							...
							<h2 className='inDashboardHeader'>
								Package ({creditPackages.length})
							</h2>
							<p className='userListAbout'>
								Create &amp; Manage new packages for Organizations
							</p>
						</div>
						<div className='listFeatures'>
							{loggedInUser && loggedInUser.organizationCode === "rbg" ? (
								<>
									<button
										className='gift-point-btn'
										onClick={() => setToggleGiftPkgDrawer(true)}
									>
										<img
											className='plusIcon'
											src={giftIcon}
									...
									<GiftPackageDrawer
										toggleDrawer={toggleGiftPkgDrawer}
										handleToggleDrawer={setToggleGiftPkgDrawer}
										toggleLoader={setIsLoader}
									/>
								</>
							) : (
								""
								...
								{creditPackages.length
									? creditPackages.map((item, index) => {
											return (
												<div
													className={
														item.isFavourite ? "cr_package fev" : "cr_package"
													}
													key={index + "_packages"}
												>
													<div className='cr_packageHead'>
														...
														<div className='cr_packageHeadRight'>
															...
															<div
																className={
																	item.status
																		? "cr_toggleBtn toggleBtn active"
																		: "cr_toggleBtn toggleBtn"
																}
																onClick={() => {
								...
								{isOrganization ? (
									...
									>
										<img
											...
											alt=''
										/>
										<span>Create the first package</span>
									</button>
								) : (
									""
								)}
