...
const CreateTagModal = (props) => {
    ...
    const createTag = async (param) => {
        if(newTagName != "") {
            try {
                let payload = {name: newTagName}
                props.loader(true);
                let response = await CustomizationServices.createTag(payload);
                dispatch({ 
        ...
        <div className="modalBackdrop cz_addTagModal">
            <div className="slickModalBody">
                ...
                <div className="modalForm">
                    <form action="">
                        <div className={formErrorMsg ? "cmnFormRow errorField" : "cmnFormRow"}>
                            <lable className="cmnFieldName">Tag Name</lable>
                            <input type="text" className="cmnFieldStyle" value={newTagName} onChange={tagNameHandel} />
                            {formErrorMsg ? <div className="errorMsg">Plese enter a tag name</div> : ""}
                        </div>
                        <div className="btnGroup centered">
                            <button type="button" className="cmnBtn" onClick={() => createTag (false)}>
                                <span>Save</span> 
                                <img src={arrowForwardIcon} alt="" />
                            </button>
                            <button type="button" className="cmnBtn" onClick={() => createTag (true)}>
                                <span>Save & New</span>
                                <img src={arrowForwardIcon} alt="" />
                            </button>
                        </div>
