...
const FilterSizesModal = (props) => {
    ...
    const handleApplyFilter = () => {
        if (selectedCat.length) {
            ...
            props.fetchProductSizes(selectedCat.toString());
        } else {
            utils.removeQueryParameter("category");
        }
    };
                ...
                <div className="sideMenuBody">
                    <form className="formBody">
                            ...
                            <div className="cz_selectCategory cmnFieldStyle" ref={ref} onClick={openCatListHandel}>
                                <div className="cz_categorySelectSec">
                                    <div className="cz_selectedCatList">
                                        <ul>
                                            {catList.length ? catList.filter(el=> selectedCat.includes(el._id)).map((item, index) => {
                                                return (
                                ...
                                <div className="cz_categoryList">
                                    ...
                                    <ul>
                                        {catList.map((item, index) => {
                                            return (
                                                <li key={"searchCat_" + index} onClick={() => selectCat (item, index)} className={selectedCat.includes(item._id) ? "disabled" : ""}>{item.name}</li>
                                            )
                                        })}
                                    </ul>
                                    : (catSearchKey ? 
                        ...
                        <div className="applyFilterBtn">
                            <button className={selectedCat.length ? "saveNnewBtn" : "saveNnewBtn disabled"} type="button" onClick={handleApplyFilter}>
                                <span>Apply Filter</span>
                                <img src={arrow_forward} alt=">" />
                            </button>
                            <button className={selectedCat.length ? "btn-link" : "btn-link disabled"} type="button" onClick={clearFilter}>Clear</button>
                        </div>
                    </form>
                </div>
            </div>
...
export default FilterSizesModal;