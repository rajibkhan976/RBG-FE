...
const CategoryListing = (props) => {
        ...
        try {
            ...
            if (!catData.name.length) {
                ...
                if (category.id) {
                    ...
                    const res = await CourseServices.editCategory(catData);
    ...
    const deleteCategory = async (catID, isConfirmed = null) => {
        ...
        if (isConfirmed == null && catID) {
            ...
            try {
                props.setIsLoader(true);
                const result = await CourseServices.deleteCategory(catID);
                if (result) {
                        ...
                        return (
                            <React.Fragment key={key + "_category"}>
                                <li ref={optionsToggleRef} className={option === key ? "active" : ""} key={elem._id}>
                                    <button className={elem.slug === "uncategorized" ? "smallListName" : "bigListName"} onClick={() => handleCategoryClick(elem._id)}>{elem.name} ({(elem.courseCount) ? elem.courseCount : 0})</button>
