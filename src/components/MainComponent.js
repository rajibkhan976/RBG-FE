...
import DocumentBuilderRouter from "./setup/documentBuilder/DocumentBuilderRouter";
...
const MainComponent = () => {
	...
	const isCreditRestriction = useSelector(
		(state) => state.credit.isRestrictionModal
	);
	...
	useEffect(() => {
		...
		socket
			.off("setNotification")
			.on("setNotification", updateSocketDataForNotification);
	...
	const updateCreditDetails = async (data) => {
		...
		if (
			loggedInUser &&
			parseData.organizationCode === loggedInUser.organizationCode &&
			parseData.type === "credit"
		) {
			dispatch({
			...
			if (userDetails) {
				let data = {
					...
					phone: userDetails.phone
						? userDetails.prefix + "-" + userDetails.phone
						: null,
					image: userDetails.image
						? config.bucketUrl + userDetails.image
						: null,
					...
					organization: userDetails.organization
						...
						: "",
					organizationCode: userDetails.organization
						? userDetails.organization.code
						: "",
					group: userDetails.group ? userDetails.group.name : "",
					isShowPlan: userDetails.organization
						? userDetails.organization.parentId !== 0
							? true
							: false
						: false,
					credit: userDetails.organization
						? userDetails.organization.credit
							? userDetails.organization.credit
							: 0
						: 0,
					currentPlan: userDetails.organization
						? userDetails.organization.package
							? userDetails.organization.package.name
							: ""
						: "",
					isPackage: userDetails.organization
						? userDetails.organization.package
							? true
							: false
						: false,
					packageId: userDetails.organization
						? userDetails.organization.package
							? userDetails.organization.package._id
							: ""
						: "",
					autoRenewLimit: userDetails.creditUsage
						? userDetails.creditUsage.autoRenewLimit
							? userDetails.creditUsage.autoRenewLimit
							: 0
						: 0,
					organizationTimezone: userDetails.organization?.timezone
						? userDetails.organization.timezone
						: "",
					organizationTimezoneInfo: userDetails.organization?.timezoneInfo
						? userDetails.organization.timezoneInfo
						: "",
				};
				dispatch({
					type: actionTypes.USER_DATA,
	...
	const scrollNotification = (type) => {
		if (type === "general") {
			if (
				notification.general.page < notification.general.totalPage &&
			...
			if (
				notification.payment.page < notification.payment.totalPage &&
				!notification.isLoading
			) {
	...
	const fetchPaymentNotification = async (page) => {
		...
		const result = await NotificationServices.fetchNotifications(
			page,
			"payment"
		);
	...
	const closeMessage = (sls) => {
		const newMessages = messages.filter(
			(mes) =>
				mes.message !== sls.message && mes.typeMessage !== sls.typeMessage
		);
			...
			<div className='mainComponent'>
				<div
					className={
						...
						(pathURL === "/automation-list" || pathURL === "/automation-builder"
							? " automationBuilderBody "
							: "") +
						(showInnerleftMenu
							? pathURL !== "/dashboard" &&
							  pathURL !== "/appointment-global" &&
							  pathURL !== "/attendance-global" &&
							  pathURL !== gj &&
							  pathURL !== "/transaction-global" &&
							  pathURL.toLowerCase() !== "/communicationlog"
								? "openSubmenu"
								: ""
							: "")
					}
				>
					...
					<div className='dashMain'>
						...
						<Switch>
							...
							<Route
								exact
								path={["/email-template", "/sms-template", "/audio-template"]}
							>
								<TemplateRoutes
									toggleLeftSubMenu={toggleLeftSubMenu}
									toggleCreate={(e) => toggleCreate(e)}
							...
							<Route
								exact
								path='/attendance-global'
							>
								<AttendanceGlobalRouting
									toggleLeftSubMenu={toggleLeftSubMenu}
									toggleCreate={(e) => toggleCreate(e)}
								></AttendanceGlobalRouting>
							</Route>
