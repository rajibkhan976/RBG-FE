...
export const utils = {
	...
	validateExpiryDate: (date) => {
		...
		let isValid =
			cardExpairySectionsMonth.length === 2 &&
	...
	convertUTCToTimezone(date, utcOffset = null, dateFormat = "LLL") {
		...
		if (!utcOffset) {
			console.log(
				"ERROR : Please send utc offset in order to convert the provided date & time"
			);
			return "Loading...";
		}
		// if (!utcOffset) return formattedDate.format(dateFormat);
		const timezoneDate = moment
			.utc(formattedDate, null)
			.utcOffset(utcOffset?.split("UTC")[1]?.replace("−", "-"));
	...
	convertTimezoneToUTC(date, utcOffset = null, dateFormat = "") {
		...
		let dateWithOffset = date.concat(
			utcOffset?.split("UTC")[1].replace("−", "-")
		);
		console.log("date with offset", dateWithOffset);
		const formattedDate = moment(dateWithOffset);
		// console.log("formatted date",formattedDate)
		if (!utcOffset)
			return "ERROR : Please send utc offset in order to convert the provided date & time";
		return moment
			.utc(formattedDate)
			.format(dateFormat)
			.replace("T", " ")
